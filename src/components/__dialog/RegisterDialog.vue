<template>
  <div class="card flex justify-center">
    ...
    >
      ...
      <div class="flex items-center gap-4 mb-8 text-red-500" v-if="store.register.isError">
        {{ t('error.' + store.register.error.response.data.message) }}
      </div>
      <form @submit.prevent="submit" class="pr-4">
        ...
        <div class="flex items-center gap-4 mb-4">
          ...
          <InputText
            ...
            autocomplete="off"
            v-model="form$.email"
            :invalid="store.register.isError"
          />
        </div>
        ...
        <div class="flex justify-end gap-2">
          <Button
            ...
            severity="secondary"
            @click="store.toggleRegisterDialog"
          ></Button>
          <Button type="submit" :label="t('register.create')" :disabled="!isFormValid"></Button>
        </div>
      </form>
    </Dialog>
  </div>
...
const submit = () => {
  store.register.mutate(form$.value)
}
</script>
